  ...
  <v-card elevation="10">
    ...
    <v-card-text v-else>
      <v-form ref="form" @submit.prevent="save">
        <v-container>
          <v-row v-if="[1, 9, 99].includes(user!.role)">
            <v-col cols="12" sm="6">
              <v-select
                v-model="item.manage_company_id"
                clearable
                label="タイヤショップ"
                placeholder="タイヤショップ"
                :items="shops"
...
import type { Field, Resource } from '@/common/defines'
const title = '顧客'
...
const fields: Field[] = [
  ...
  },
  {
    ...
    title: '電話番号',
    editor: 'tel',
    rules: [(v) => /^[0-9]*$/.test(v) || '半角数字のみを入力してください'],
    search: true
  },
  {
    key: 'address',
...
const loadShops = async () => {
  if (!shops.value.length) shops.value = [11, 19].includes(user!.role) ? [user!.company] : (await api.all('shop')).data
  item.value.manage_company_id = shops.value[0].id as string
}
loadShops()
